...
cvcalc::cvcalc(){
 sub = nh.subscribe<sensor_msgs::Image>("camera/image_raw", 1, &cvcalc::rgbImageCallback, this);
...
void cvcalc::judge_enemy(){
 if (std::isnan(cog.x) || (cog.x ==0 && cog.y ==0)){
  status = false;
  place = 0;
 }else{
  status = true;
...
void cvcalc::pub_info(){
  cvtest::cv_msg cv_msg;
  cv_msg.status = status;
  cv_msg.isdoubleget = isdoubleget;
  cv_msg.place = place;
  cv_msg.enemy_vel = enemy_vel;
  cv_msg.enemy_dis = enemy_dis;
  pub.publish(cv_msg);
}
